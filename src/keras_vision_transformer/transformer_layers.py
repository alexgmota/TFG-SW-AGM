...
class patch_expanding(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, upsample_rate, return_vector=True, name=''):
        ...
        self.linear_trans1 = tf.keras.layers.Conv2D(upsample_rate*embed_dim, kernel_size=1, use_bias=False, name='{}_linear_trans1'.format(name))
        # 
        self.linear_trans2 = tf.keras.layers.Conv2D(upsample_rate*embed_dim, kernel_size=1, use_bias=False, name='{}_linear_trans1'.format(name))
        self.prefix = name
